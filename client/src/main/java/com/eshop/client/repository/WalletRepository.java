...
public interface WalletRepository extends BaseRepository<WalletEntity, Long> {
	...
	@Query("SELECT new com.eshop.client.model.BalanceModel(w.currency, "
			...
			+ "FROM WalletEntity w WHERE w.user.id = :userId AND (:createdDate is null or DATE(w.createdDate) = :createdDate) AND w.active is true GROUP BY w.currency")
	 List<BalanceModel> totalProfitGroupedByCurrency(UUID userId, LocalDateTime createdDate);
